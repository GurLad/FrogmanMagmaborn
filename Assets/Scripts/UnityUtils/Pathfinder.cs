...
{
    public static float GetTrueDistance(Unit unit, Vector2Int start, Vector2Int end)
    {
        if (start == end)
        {
            return 0;
        }
        List<Vector2Int> parts = GetPath(unit, start, end);
        float sum = 0;
    ...
    public static List<Vector2Int> GetPath(Unit unit, Vector2Int sourceVec, Vector2Int destinationVec)
    {
        // From Wikipedia...
        ...
        if (!CanMove(unit, destination.x, destination.y))
        {
        ...
        while (openSet.Count > 0)
        {
            ...
            {
                if (CanMove(unit, neighbor.x, neighbor.y))
                { 
                    int tentativeScore = gScore[current] + GetDistance(unit, current, neighbor); // No safe as the current should always have a gValue
    ...
    private static int GetDistance(Unit unit, Node current, Node neighbor)
    {
        return GameController.Current.Map[neighbor.x, neighbor.y].GetMovementCost(unit);
    }

    private static bool CanMove(Unit unit, int x, int y)
    {
